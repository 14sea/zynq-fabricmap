...
| `make_archive.py` | the producer (reads the working tree; refuses unless HEAD descends from the base, the B2 manifest and table are the completion-state documents, and every file hashes to its pin) |
...
| `controls.py` → `runs/` | the positive run under `env -i PATH=…` in a fresh checkout, and the negative controls (member missing / one byte changed, each with the original manifest and with the outer digest patched; a target already holding the files; a target not at the base); `runs/summary.json` |
