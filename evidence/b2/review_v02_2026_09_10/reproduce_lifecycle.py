...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="b2_review_v02_") as td:
        ...
        check("baseline_S3", bm.pin_plan(qualified, plan_dir / "plan.json"))
        mutations = {
        ...
        for name, mutate in mutations.items():
            ...
            check("pin_plan_accepts_" + name, bm.pin_plan(qualified, path))
        ...
        path.write_text(json.dumps(p))
        planned = bm.pin_plan(qualified, plan_dir / "plan.json")
        ...
        results["slow_positive_rate"] = {"rate_per_hour": 100, "status": split["status"],
            "expected_span_s": split["sessions"][0]["expected_span_s"], "registered_max_s": bp.SESSION_SPAN_MAX_S}
