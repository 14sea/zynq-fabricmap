"""Read-only audit of B1Q attempt 4. Writes results only to a separate output directory."""
...
D = R / 'evidence/b1q/b1q_17A6_2026-09-08-01'
O = Path('/tmp/b1q_session4_audit')
...
differences(m, refreshed)
assert changed == ['carrier.qualification', 'carrier.qualified'], changed
prospective_sha = hashlib.sha256((json.dumps(refreshed, indent=1, ensure_ascii=False)+'\n').encode()).hexdigest()
# All originals must still be exactly the bytes committed for this attempt.
for path in [*D.iterdir(), R/'evidence/b1q/principal_boundary_2026-09-08-01.json']:
    if path.is_file():
        assert path.read_bytes() == subprocess.check_output(['git', 'show', 'bc85b6c:'+str(path.relative_to(R))])
...
boundary=load(R/'evidence/b1q/principal_boundary_2026-09-08-01.json')
...
rulings={}
for key,name in [('whole_of_run','b1q_2026-09-08-01.json'),('provisioning','p3_k_b1q_2026-09-08-01.json')]:
