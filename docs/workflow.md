...
- **Git:** branch `main` (the repo was created on `master` and renamed 2026-08-02 to
  match the sibling repos; no remote existed, so nothing was rewritten), commit
  locally without asking, **always ask the human before pushing**. No remote exists
  yet — creating one is a human decision.
