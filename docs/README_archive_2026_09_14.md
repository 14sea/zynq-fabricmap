# README archive — the running log as it stood on 2026-09-14

> Preserved verbatim when the top-level README was replaced by a short one (S2 qualify day).
> Nothing below was edited; dates, states and links are as written at the time.
> The current summary is `../README.md`.
...
> **▶ 2026-09-06, B1Q attempt 2 on `17A6`: PASS (11/11 audited, exports sealed, the gate observations as predicted; owner's audit PASS) — but not the final qualification: pinning its record would go red on two pinned tests, and fixing them changes the pin table, which the strict transition rule rightly refuses. Owner's decision: keep the rule; attempt 2 stays a valid PASS under its manifest; the new manifest is re-qualified under an attempt-3 pair. v2.4.3 (local): the tests assert the freeze binding only, and `host/b1_qualified_state_check.py` proves the suite in both the unqualified and a pinned-record state before any board session — [`docs/b1q_transition_decision_2026_09_06.md`](docs/b1q_transition_decision_2026_09_06.md).**
>
> **▶ 2026-09-06, v2.4.2 (local): the owner's review of v2.4.1 held it on two more export defects — the manifest check validated only what the document listed, and a manifest write failure escaped the session's `finally` without a summary. Now the manifest is checked against its declared schema (five physical files, six statuses, hashes and sizes) in both adjudicators, the seal is isolated with its failures recorded, and the summary always lands — [`docs/b1_v241_review_2026_09_06.md`](docs/b1_v241_review_2026_09_06.md).**
>
> **▶ 2026-09-06, v2.4.1 (local): the owner's review of v2.4 held it on two export defects — a failed raw-console write could still PASS, and a helper failure inside run_log / audits dropped collected records or chunks. Now every required export must be complete (`exports.json`, checked by both adjudicators and bound by the qualification chain), base data is written independently of each enrichment (PARTIAL → HOLD), and the post-go exception path is tested through the real `b1_session.run` — [`docs/b1_v24_review_2026_09_06.md`](docs/b1_v24_review_2026_09_06.md).**
>
