...
| map | `evidence/b1/b1_17A6_2026-09-08-02/self_map_v2.json`: canonical-JSON sha256 `c6a4b23e1871e1889621191d671886f804c88edaa9748119d9a5006fd0b64296` (sorted keys, no spaces — what the IDENT carries and the Python consumers compare), file-byte sha256 `b6607a9a4de4ec16b4441395703d353ba438463e3ff0be3b5984ec6f1f1bfa38` (what a file pin checks); both in the manifest. Compiled into the image as the column table (`init_index` per genome bit — no LUT key) |
...
| session split (frozen rule) | `host/b2_plan.session_split`: a session holds the largest whole number of pairs whose **expected span** `records × 3600 / R` ≤ 7 200 s (at least one), pairs assigned in order, each session with its own baselines; **R = the B2Q-measured all-self-reporting rate** written into the manifest's `calibration` (S2). Until then the split is **UNDETERMINED** (the plan says so). Planning rates only, not calibration: S #3 sampled ≈ 6 690 / h, B1's plan 3 368 / h, the last B1 mapping ≈ 2 807 / h observed — at 2 807 / h the rule gives 4 pairs per session, three sessions (4 + 4 + 1), 10 824 records; whether two sessions suffice is **not assumed** |
...
| **S2 qualify** | the tool, after B2Q PASS | the B2Q record (files by hash, binding to `manifest_at_run`, outcome, measured rate) **and** `calibration` from that rate, in one licensed transition | the current manifest must equal `manifest_at_run` outside {qualification, qualified, calibration, plan, status, history}; the record's binding must be this manifest's image / prereg / carrier / map; re-adjudication must PASS (pluggable until the B2 adjudicator exists; without it, not qualified) |
| **S3 plan** | the tool | the plan regenerated with `--rate-per-hour` = the calibration and pinned (path, sha256, prediction sha256, sessions, total records) | the plan's split must equal `session_split(9, 600, R)`; a plan on an unqualified manifest, a second plan, a plan with another rate: refused |
| **B2 ruling pairs** | the owner | bound to the **S3** manifest's sha256 (session B2, master seed, image, prereg) | — |
| any later change | — | image, prereg, map, seeds, lineage, experiment, calibration, plan file, pin, a lying `qualified` flag | **refused** by `verify` (each case a fresh-process test) |
