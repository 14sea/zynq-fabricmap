...
| `b3/host/b3_carto.py`, `b3_online_arm.py`, `b3_control_x.py`, `b3_online_map.py`, `b3_gate.py`, `b3_gate_report_md.py`, `b3_plan.py`; `b3/schemas/`; `b3/tests/` | written in lifecycle 1 (reviewed to `25c7ede`, `8b43205`, `b1b82d0`); brought under this v0.3 by the lifecycle-2 code unit (`c0a8bf9` and its follow-up to the owner's HOLD: the changes of preregistration §10 (i)–(v); the gate refuses a dirty or uncommitted tree, binds every raw file by digest and is read by the plan and the renderer only through a validator that re-runs `evaluate` from the raw rows; the H9 diagnostic reports no "power at N₂" when N₂ does not exist; lifecycle 1's directories and everything under them are refused as outputs; refusals are named `REFUSED:` exits) | the implementation, under the pin rule `b3/**/*`; the lifecycle-2 gate run and the prediction preflight are next |
