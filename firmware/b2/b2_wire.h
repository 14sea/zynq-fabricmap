/* b2_wire — the B2 image's wire serialisation, as a pure unit.
 *
 * DERIVED from firmware/b1/b1_wire.c/h (itself derived from the instrument's p3_wire) with
 * exactly two changes, recorded in firmware/b2/IMPORT.json: app_identity becomes 1.5.0 and
 * carries what stage B2 searches with (engine, map digest, fitness, per-arm budget and this
 * session's pair slice), and the loop record's pre-rendered block is the `search` block of
 * b2_search.c instead of B1's `carto`. Everything else — the framed line, the sign request,
 * the evidence members and their sorted-key order — is B1's byte for byte.
 *
 * The original rationale, which still holds:
 *
...
typedef struct {
    ...
    /* app_identity 1.4.0 (B1): the universe digest the image was compiled with, and the
     * carrier VARIANT register as read. B1's `carto_version` and `probe_budget` are NOT
     * carried: this image runs no cartographer and issues no probes, and an identity that
     * declared them empty would describe an image that does not exist. */
    const char *universe_sha256;      /* 64 hex, B2_UNIVERSE_SHA256 */
    uint32_t carrier_variant;         /* the VARIANT register as read (0x42310001) */
    /* app_identity 1.5.0 (B2): what this image searches with — the engine, the map it was
     * compiled with, the fitness, the per-arm budget, and the pairs the identity page gave
     * this SESSION out of the preregistered total (the split of preregistration §2) */
    const char *search_version;       /* B2_SEARCH_VERSION */
    const char *map_sha256;           /* 64 hex, B2_MAP_SHA256 — the B1 self-map's canonical digest */
    const char *fitness_id;           /* "F1" */
    uint32_t budget_per_arm;
    uint32_t pairs_total;             /* the preregistered N */
    uint32_t pair_first, pair_count;  /* this session's slice */
} p3_wire_identity_in;
...
typedef struct {
    ...
    const char *search;   /* loop_record 1.3.0 (B2): the search's pre-rendered JSON block (sorts
                           * between schema_version and seq), or NULL on a baseline */
