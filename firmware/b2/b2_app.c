/* b2_app — the B2 standalone application: the board half of the map-utility session
 * (docs/b2_architecture.md, docs/b2_preregistration.md), a versioned SUCCESSOR of the B1
 * application (firmware/b1/b1_app.c), which is itself derived from the instrument's p3_app.c
 * (zynq-psoracle 689dde1). firmware/b2/IMPORT.json names the derivation.
 *
 * ─── STANDING OF THIS FILE ────────────────────────────────────────────────────────────
 * COMPILED, NOT BOARD-RUN. It has NEVER been run on a board; no B2 ruling exists, the image
 * is not `board_ready`, and the first board contact would be the image qualification session
 * B2Q under its own ruling pair (docs/b2_preregistration.md §6).
 * What IS proven host-side: p3_derive.c (verbatim) against the instrument's Python reference;
 * b2_search.c and b2_orch.c against the Python reference evaluation by evaluation and
 * candidate by candidate, including every `search` record block byte for byte and the
 * preregistered deltas (tests/test_b2_twin.py, tests/test_b2_session.py); the wire bytes this
 * file emits through b2_wire.c against the instrument's validator (tests/test_b2_wire.py).
 * This file is deliberately thin for that reason: HAL plus state machine.
 * ──────────────────────────────────────────────────────────────────────────────────────
 *
 * The diff to b1_app.c is: the cartographer/orchestrator replaced by the SEARCH orchestrator
 * (b2_orch.c over b2_search.c), the `carto` block replaced by the `search` block in every
 * record, the identity's B2 fields, and the session's pair SLICE decoded from the identity
 * page's flags word. Where this file and the Python reference (host/b2_session.run) disagree,
 * the Python is the specification. The session:
 *
 *   identity → orchestrator init (the pair seeds derived on the board) → opening baseline
 *   (its measured readout is every arm's base) → [ propose → notary (ZERO tables) → stage →
 *   link 2 → DMA → link 3 → ARM → raw readout → observe → the fitness computed HERE from
 *   that measured readout ]* → closing baseline (= restore) → closing unsigned ARM
 *
 * ending in exactly one of COMPLETED / STOPPED / PROTOCOL. An unscored candidate ends the
 * epoch: nothing is re-issued and no closing baseline follows.
 */
...
#define B2_VARIANT_WORD 0x42310001u /* the B1 carrier, contract v1: SEMANTIC_GATE = 0 —
                                    * B2 runs on it unchanged (docs/b2_architecture.md D1) */
...
/* B2: the on-board SEARCH replaces B1's cartographer (b2_search.c, driven by b2_orch.c).
 * The identity page's seed is the MASTER seed the board derives all pair seeds from, its
 * budget is the per-arm budget, and its flags carry this session's pair slice; flags bits 2-3
 * (L6's schedule mode) are ignored. Every non-baseline record carries the `arm` field and the
 * `search` block; a baseline carries neither. */
static b2_orch O;                          /* the session orchestrator: the two arms + the order */
static char g_search_json[2048];
...
static int establish_identity(void)
{
     ...
     * requires app_identity and this application never sent one. */
    {
        ...
        in.findings_n = nf;
        /* app_identity 1.5.0 (B2): what this image searches with. The master seed is the one
         * every pair seed is derived from ON THE BOARD; the map digest is the B1 self-map
         * compiled in; the pair slice is this SESSION's share of the preregistered total. */
        ...
        in.budget_per_arm = S.page.budget;
        in.pairs_total = (uint32_t)g_pairs_total;
        in.pair_first = (uint32_t)g_pair_first;
        in.pair_count = (uint32_t)g_pair_count;
 ...
 * and it is marked only once that baseline has been OBSERVED (b2_orch_complete); the opening
 * one leaves the orchestrator in its first search phase (the
...
static void note_scored(int is_baseline, const char *commit, const char tables[6][17])
{
    ...
    S.have_last_reply = 1;
    if (is_baseline && b2_orch_complete(&O))
                         ...
                         const char *arm_name)
{
    ...
    }
    {
        ...
        for (i = 0; i < 6; i++) {
            ...
            g_tables_u64[i] = ((uint64_t)hi << 32) | (uint64_t)lo;
        }
        rec.hb_before = hb_before;
        rec.hb_after = axi_read(P3_HEARTBEAT);
        /* B2: the fitness is computed HERE, from the PL's own readout of this candidate —
         * never from a model of it — and the record carries the search state it produced */
        b2_orch_observe(&O, S.seq, g_tables_u64);   /* a baseline sets the base; a candidate is scored */
        rec.search = search_block();
 ...
 *            and ANY candidate that is not SCORED ends the epoch; the closing unsigned
 *            control and COMPLETED only when the closing baseline was observed;
...
static void b2_session_init(void)
{
    ...
    if (b2_orch_init(&O, S.page.seed, S.page.budget, g_pairs_total, g_pair_first, g_pair_count,
                     S.page.token, B2_UNIVERSE_SHA256, S.page.app_image_sha_lo32) != 0)
        p3_stop(P3_STOPPED, "STOP_PAGE: the identity page's session parameters were refused");
}

static void b2_session_run(void)
{
    uint32_t genome[P3_GENOME_WORDS];
    int is_baseline;
    while (S.kind == P3_RUNNING && b2_orch_next(&O, genome, &is_baseline)) {
        if (run_candidate(genome, is_baseline, b2_orch_arm_name(&O)) != 0) {
            b2_orch_unobserved(&O);
            break;
        }
    }
    if (S.kind == P3_RUNNING && b2_orch_complete(&O)) {
...
static void b2_session_finish(void)
{
...
int main(void)
{
    ...
    memset(blank, 0, sizeof(blank)); /* the blank genome IS the pinned base (b2_orch proposes it) */
    (void)blank;
    (void)genome;
