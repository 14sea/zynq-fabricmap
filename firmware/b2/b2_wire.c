...
size_t p3_wire_identity(const p3_wire_identity_in *in, char *out, size_t max)
{
    ...
    w_init(&w, out, max);
    /* 1.5.0 (B2): budget_per_arm sorts between app_epoch and carrier_sha256 */
    w_fmt(&w, "{\"app_epoch\":%lu,\"budget_per_arm\":%lu,\"carrier_sha256\":",
          (unsigned long)in->app_epoch, (unsigned long)in->budget_per_arm);
    w_str(&w, in->carrier_sha256);
    /* 1.4.0 (B1): carrier_variant sorts between carrier_sha256 and control_plane */
    ...
    /* sorted keys, as every payload: fitness_id < map_sha256 < master_seed < nonce_at_start
     * < operator_data_sha256 < pair_count < pair_first < pairs_total < protocol
     * < pss_idcode < rec_retry_control < schedule_mode < schema */
    w_fmt(&w, "],\"fitness_id\":");
    w_str(&w, in->fitness_id ? in->fitness_id : "");
    w_fmt(&w, ",\"map_sha256\":");
    w_str(&w, in->map_sha256 ? in->map_sha256 : "");
    ...
    /* 1.5.0 (B2): the session's pair slice sorts between operator_data_sha256 and protocol */
    w_fmt(&w, ",\"pair_count\":%lu,\"pair_first\":%lu,\"pairs_total\":%lu",
          (unsigned long)in->pair_count, (unsigned long)in->pair_first, (unsigned long)in->pairs_total);
    w_fmt(&w, ",\"protocol\":");
    ...
    /* 1.5.0 (B2): search_version sorts between schema_version and sign_retry_control */
    w_fmt(&w, ",\"schema\":\"app_identity\",\"schema_version\":\"1.5.0\",\"search_version\":");
    w_str(&w, in->search_version ? in->search_version : "");
    w_fmt(&w, ",\"sign_retry_control\":%s,"
...
size_t p3_wire_loop_record(const p3_wire_record_in *in, char *out, size_t max)
{
    ...
    w_fmt(&w, ",\"outcome\":");
    w_str(&w, in->outcome);
    /* 1.3.0 (B2): "search" sorts between "schema_version" and "seq"; pre-rendered JSON */
    w_fmt(&w, ",\"schema\":\"loop_record\",\"schema_version\":\"1.3.0\"");
    if (in->search != NULL)
        w_fmt(&w, ",\"search\":%s", in->search);
    w_fmt(&w, ",\"seq\":%lu,\"verified\":\"%s\"}",
