...
>
> **▶ 2026-09-05, later: the round 1′ package is delivered at "ready for the board" and STOPPED there for the whole-package review — [`docs/claimb_round1prime_package.md`](docs/claimb_round1prime_package.md).** Preregistration DRAFT (not frozen), plan and prediction pinned, runner fail-closed, 57 tests. **Read its §0 first:** the host model shows the carrier's fitness is additive over the 292 bits, so Claim B's outcome on this instrument is decided by arithmetic (the prediction: every primary block tied at +4, falsifier 1 fires) and no operator can have navigational value here; the board run would measure that negative and 11 754 predicted known answers through the oracle, nothing about the map. Still no board contact.
>
